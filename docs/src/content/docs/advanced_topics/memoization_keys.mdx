...
For types you don't control, register the ID with `register_memo_key_function()`:
...
- **Inheritance**: Declaring `__coco_memo_type_id__` on a base class propagates to subclasses. A subclass can override the ID by declaring its own.
- **Precedence**: A declared `__coco_memo_type_id__` (own or inherited) takes precedence over any registered stable type ID.
- **MRO registration**: Registered stable type IDs also apply to subclasses across the MRO: the most specific registered type wins. A subclass can override an inherited registered ID by declaring or registering its own.
...
A class object is the class value itself (such as `ProductRow`), not an instance (such as `ProductRow(...)`).

When CocoIndex fingerprints a class object:

* It uses the class's stable type ID (declared with `__coco_memo_type_id__` or registered with `register_memo_key_function()`) when defined, or falls back to its module and qualified name (`module.QualName`).
* It does not call `__coco_memo_key__` or `__coco_memo_state__` methods defined on the class or its metaclass.
* It fingerprints only the class identity, not its schema or attributes. Include class details with `memo_key=` if the function depends on them.
* To customize memoization for class objects globally, register a memo key function on the class's custom metaclass or on `type` (which takes precedence over stable type IDs).
